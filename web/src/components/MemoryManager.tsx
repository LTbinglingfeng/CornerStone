  ...
  return (
    <div className="memory-manager">
      <div className="memory-header">
        <h3>记忆管理</h3>
        <button className="add-btn" onClick={() => setShowAddModal(true)}>+ 添加</button>
      </div>

      <div className="memory-hint">
        提示：记忆会保存在本地。开启长期记忆后，系统会将最近 5 轮对话片段发送给记忆处理模型用于提取，请勿输入敏感信息。
      </div>
  ...
  return (
    <div className="memory-item">
      <div className="memory-content">
        ...
        <span className="memory-text" title={memory.content}>{memory.content}</span>
      </div>
      <div className="memory-meta">
        ...
        <button className="delete-btn" onClick={onDelete} title="删除">×</button>
      </div>
    </div>
  )
}
...
function MemorySection({
  ...
  return (
    <section className="memory-section">
      ...
      {!collapsed && memories.map((m) => (
        <MemoryItem key={m.id} memory={m} onDelete={() => onDelete(m.id)} />
      ))}
    </section>
  )
}
