...
const ClawBotSettingsPanel: React.FC<ClawBotSettingsProps> = ({ onBack }) => {
    ...
    const statusLabel = statusLabelMap[settings?.status || ''] || settings?.status || '未配置'
        ...
        >
            ...
            {loading ? (
                ...
                <div className="provider-settings-content">
                    <div className="clawbot-status-card">
                        <div className="clawbot-status-top">
                            <div className="clawbot-status-title">运行状态</div>
                            <div className={`clawbot-status-pill ${settings?.status || 'disabled'}`}>{statusLabel}</div>
                        </div>
                        <div className="clawbot-status-detail">
                            {settings?.polling ? '后台轮询中' : '当前未轮询'}
                            {settings?.prompt_name ? ` · 人设：${settings.prompt_name}` : ''}
                        </div>
                        {settings?.ilink_user_id && (
                            <div className="clawbot-status-subdetail">iLink User ID: {settings.ilink_user_id}</div>
                        )}
                    ...
                    <div className="settings-group">
                        ...
                        <div className="modal-toggle-wrapper">
                            ...
                            <span className="toggle-label">{form.enabled ? '开启' : '关闭'}</span>
                        </div>
                        <p className="prompt-modal-hint memory-toggle-hint">聊天仅保存在内存，不会写入聊天记录文件</p>
                    </div>
                    ...
                    <div className="settings-group">
                        ...
                        <input
                            ...
                            onChange={(event) => setForm((current) => ({ ...current, bot_token: event.target.value }))}
                        ...
                        {settings?.has_bot_token && (
                            ...
                            >
                                {form.clear_bot_token ? '已标记清空 Bot Token' : '清空已保存 Bot Token'}
                            </button>
                        )}
                        <div className="clawbot-token-hint">
                            {settings?.has_bot_token
                                ? `当前已保存：${settings.bot_token || '****'}`
                                : '当前未保存 Bot Token'}
                        </div>
                    </div>
